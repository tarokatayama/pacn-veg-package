# Load the Shiny and plotly libraries
library(shiny)
...
server <- function(input, output) {
  ...
  output$utilities_chart <- renderPlotly({
    data <- data.frame(
      Category = c("Electricity", "Water", "Power", "Internet"),
      Cost = c(input$electricity, input$water, input$power, input$internet)
    )
    
    plot <- plot_ly(data, x = ~Category, y = ~Cost, type = "bar", name = "Cost") %>%
      layout(title = "Monthly Utilities",
             xaxis = list(title = ""),
