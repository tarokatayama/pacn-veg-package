...
server <- function(input, output, session) {
  ...
  # Calculate the total cost for the selected month
  total_cost <- reactive({
    if (nrow(all_data()) > 0) {
      recent_month <- input$month
      recent_data <- subset(all_data(), Month == recent_month)
      sum(recent_data$Electricity, recent_data$Water, recent_data$Power, recent_data$Internet)
    } else {
      0
    }
  })
  ...
  # Reset all user inputs when the "Reset" button is clicked
  observeEvent(input$reset, {
    updateSelectInput(session, "month", selected = "August")
    updateNumericInput(session, "electricity", value = 0)
    updateNumericInput(session, "water", value = 0)
    updateNumericInput(session, "power", value = 0)
    updateNumericInput(session, "internet", value = 0)
    all_data(data.frame(Month = character(0), Electricity = numeric(0), Water = numeric(0), Power = numeric(0), Internet = numeric(0)))
  })
}
